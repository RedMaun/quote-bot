...
class Command(AbstractCommand):
    def __init__(self):
        super().__init__(handler = ['/еврей <k> <item>', '/ЕВРЕЙ <k> <item>', '/еврей <item>', '/ЕВРЕЙ <item>', '/еврей', '/ЕВРЕЙ'], description = 'jewificator v1.756')
...
async def jewificator(m: Message, k: Optional[str] = None, item: Optional[int] = None):
    try:
        if m.reply_message:
            ...
            print(k)
            if k == None:
                answer = "("*3 + m.reply_message.text + ")"*3
                await Jew.ans_up(answer, m)
            elif int(k) < 100 and int(k) > 0:
                k = int(k)
                answer = "("*k + m.reply_message.text + ")"*k
                await Jew.ans_up(answer, m)
        else:
