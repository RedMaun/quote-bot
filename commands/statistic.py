...
async def statist(m: Message, n: Optional[int] = None, item: Optional[int] = None):
    ...
    try:
        if (n != None and int(n) == 1):
            ...
            start = datetime.datetime.strptime(_dates[0], "%d.%m.%Y")
            end = datetime.datetime.strptime(datetime.date.today().strftime("%d.%m.%Y"), "%d.%m.%Y")
            date_generated = [start + datetime.timedelta(days=x) for x in range(0, (end-start).days)]

            dates_gen_parse = []
            for datee in date_generated:
                dates_gen_parse.append(datee.strftime("%d.%m.%Y"))
            ...
            h = []
            if (item != None):
                if (int(item) >= len(dates_gen_parse)):
                    for i in range(len(dates_gen_parse)):
                        b.append(_dates.count(dates_gen_parse[i]))
                        h.append(dates_gen_parse[i])
                else:
                    for i in range(len(dates_gen_parse)-int(item), len(dates_gen_parse)):
                        b.append(_dates.count(dates_gen_parse[i]))
                        h.append(dates_gen_parse[i])
            else:
                for i in range(len(dates_gen_parse)):
